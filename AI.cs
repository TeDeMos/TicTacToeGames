...
namespace TicTacToe
{
    public class AI
    {
        ...
        private IntPoint MakeBigGridChoice(BigGrid bg)
        {
            ...
            {
                IntPoint secondChoice = MakeSmallGridChoice(bg.Board[game.BigGridCurrentPosX, game.BigGridCurrentPosY]); //Select a random cell from the only available small grid
                return new IntPoint(3 * game.BigGridCurrentPosX + secondChoice.X, 3 * game.BigGridCurrentPosY + secondChoice.Y);
            }
        }
        ...
        private IntPoint MakeHugeGridChoice()
        {
            ...
            {
                IntPoint secondChoice = MakeBigGridChoice(hugeGrid.Board[game.HugeGridCurrentPosX, game.HugeGridCurrentPosY]); //Select a random cell from the only available big grid
                return new IntPoint(9 * game.HugeGridCurrentPosX + secondChoice.X, 9 * game.HugeGridCurrentPosY + secondChoice.Y);
            }
        }
    }
}
